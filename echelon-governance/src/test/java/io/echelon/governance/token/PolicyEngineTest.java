...
class PolicyEngineTest {
    ...
    private final PolicyStore testStore = new PolicyStore() {
        @Override public List<DeonticToken.Permit> getPermits(String role) {
            if ("implementer".equals(role)) {
                return List.of(new DeonticToken.Permit("write_source", Set.of("implementer"), "core", "test", NOW));
            }
            return List.of();
        }
        @Override public List<DeonticToken.Embargo> getEmbargoes(String role) {
            if ("implementer".equals(role)) {
                return List.of(new DeonticToken.Embargo("write_to_main", Set.of("implementer"), "core", "test", NOW));
            }
            return List.of();
        }
